...
- 時間波形CNNへFFT枝を加えたモデルも`uno_q_fft_candidates/`に生成した。標準FP16版は生の512点を入力し、TFLite内部でHann窓付きRFFTを実行する。時間CNN 85%＋FFT標準版15%のensemble（5.57 MB）は、従来CNNに対して60点top-1 97.95%→98.01%、12エリア98.85%→98.95%、Expected XY 1.86→1.83 mmへ改善した。融合比率は検証セッションのみで決定した。FFT標準単体はMAP平均誤差を1.91→1.56 mmへ改善し、590万パラメータの大型版は改善しなかった。FFT標準版、従来時間波形CNN版、CNN＋FFT ensemble版の3モデルをUNO Qへ配備し、確率演奏画面の「推論モデル」から切替可能にした。全interpreterは起動時に生成・ウォームアップするため、打点ごとのモデル起動オーバーヘッドはない。従来CNNだけは512点波形から448点×3チャンネル特徴量を実行時生成する。選択は`data/config/inference_model.json`へ保存され、既定値はFFT標準版である。詳細は [UNO Q FFT内蔵ハイブリッドモデル評価](uno-q-fft-hybrid-evaluation-20260911.md) を参照する。
...
- `acrylic_pan_fft_hybrid_standard_fp16`、`acrylic_pan_xy_gpu_fp16`（従来CNN）、`acrylic_pan_temporal_fft_ensemble_fp16`の3モデルがLiteRT CPU/XNNPACKでロードされ、APIとWeb UIから動作中に切替できる。既定はFFT標準版である。
