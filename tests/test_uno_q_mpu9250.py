...
class UnoQMpu9250Tests(unittest.TestCase):
    ...
    def test_static_pages_are_complete(self):
        ...
        probability_css = (static / "instrument-probability.css").read_text(encoding="utf-8")
        ...
        self.assertIn('value="fft_large"', probability_html)
        ...
        self.assertIn("socket.io/socket.io.js", probability_js)
        self.assertIn("socket.on('image'", probability_js)
        self.assertNotIn("socket.on('classification'", probability_js)
        self.assertIn("CAMERA_SOURCE_KEY", probability_js)
        self.assertIn("let cameraSource = 'unoq'", probability_js)
        self.assertIn("CAMERA_ENABLED_KEY", probability_js)
        self.assertIn("if(cameraEnabled)return startCamera()", probability_js)
        self.assertIn("localStorage.setItem(CAMERA_ENABLED_KEY,'false')", probability_js)
        self.assertIn('id="cameraStop" type="button" disabled>カメラOFF</button>', probability_html)
        self.assertIn("prepareUnoQAudio()", probability_js)
        self.assertNotIn("current.connected&&current.inference_active)await api('/api/inference/stop'", probability_js)
        ...
        self.assertIn("aspect-ratio:4/3", probability_css)
        self.assertIn("width:100%;height:100%", probability_css)
        self.assertIn("object-fit:cover;transform:none", probability_css)
        self.assertIn("area-probability-card", probability_html)
        self.assertIn("grid-template-rows:repeat(12,minmax(0,1fr))", probability_css)
    ...
    def test_uno_q_retrigger_guard_matches_original_application_contract(self):
        ...
        main = (ROOT / "uno_q_app" / "python" / "main.py").read_text(encoding="utf-8")
        server = (ROOT / "uno_q_app" / "python" / "web_server.py").read_text(encoding="utf-8")
        instrument = (ROOT / "uno_q_app" / "python" / "static" / "instrument.js").read_text(encoding="utf-8")
        probability = (ROOT / "uno_q_app" / "python" / "static" / "instrument-probability.js").read_text(encoding="utf-8")
        self.assertIn('Bridge.provide("set_retrigger_guard"', sketch)
        self.assertIn("retriggerGuardMs = 120", sketch)
        self.assertIn('"retrigger_guard_ms": 120', main)
        self.assertIn("retriggerGuardMs:120", instrument)
        self.assertIn("milliseconds:120", probability)
        ...
        self.assertIn("nextStatusAt = millis() + 20000", sketch)
