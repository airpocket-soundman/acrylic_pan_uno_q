...
const CAMERA_ENABLED_KEY = 'acrylicPanProbabilityCameraEnabled';
...
let cameraSocketClientPromise = null;
let cameraEnabled = localStorage.getItem(CAMERA_ENABLED_KEY)!=='false';
// Start in appliance mode. The input selector can still switch to a PC camera
// after load, but a fresh visit uses the stream hosted by UNO Q.
let cameraSource = 'unoq';
...
function populateSongOptions(){const selected=activeSongId||$('songSelect').value;$('songSelect').replaceChildren(new Option(L('自由演奏','Free play'),''),...Object.entries(SONGS).map(([id,song])=>new Option(songName(song)+L('（16小節）',' (16 bars)'),id)));$('songSelect').value=SONGS[selected]?selected:'';}
function renderPerformanceStatus(){const message=performanceMessage;if(message.kind==='note')$('performanceStatus').textContent=L('演奏: エリア'+(message.area+1)+' / '+message.note+' / エリア確率 '+message.percent+'%','Playing: Area '+(message.area+1)+' / '+message.note+' / Area probability '+message.percent+'%');else if(message.kind==='running')$('performanceStatus').textContent=L('確率分布演奏中です。最尤エリアを発音します。','Probability performance is active. The most likely area will sound.');else if(message.kind==='ready')$('performanceStatus').textContent=L('推論・カメラ・音源を準備中です。ブラウザの制限解除のため「演奏開始」を1回押してください。','Inference, camera, and sounds are being prepared. Press Start performance once to unlock browser audio.');else if(message.kind==='stopped')$('performanceStatus').textContent=L('演奏を停止しました。','Performance stopped.');else $('performanceStatus').textContent=L('「演奏開始」を押すと音声と確率推論が有効になります。','Press Start performance to enable audio and probability inference.');}
...
function applyLanguage(){
  ...
  [['refresh','更新','Refresh'],['connect','接続','Connect'],['disconnect','切断','Disconnect'],['performanceStart','演奏開始','Start performance'],['performanceStop','演奏停止','Stop performance'],['probabilityDemo','表示デモ','Display demo'],['cameraStart','カメラON','Camera ON'],['cameraStop','カメラOFF','Camera OFF'],['calibrationStart','8点フィット','8-point fit'],['calibrationClear','フィット消去','Clear fit'],['songReset','最初から','Restart']].forEach(([id,ja,en])=>$(id).textContent=L(ja,en));
...
async function loadUnoQAudio(note,area,duration){
  ...
  if(!unoQAudioLoads.has(url))unoQAudioLoads.set(url,(unoQAudioBytes.has(url)?Promise.resolve(unoQAudioBytes.get(url)):fetch(url,{cache:'force-cache'}).then(response=>{
    if(!response.ok)throw new Error(`${response.status} ${response.statusText}`);
    return response.arrayBuffer();
  }).then(bytes=>{unoQAudioBytes.set(url,bytes);return bytes;})).then(bytes=>audioContext.decodeAudioData(bytes.slice(0))).then(buffer=>{unoQAudioBuffers.set(url,buffer);unoQAudioLoads.delete(url);return buffer;}).catch(error=>{unoQAudioLoads.delete(url);throw error;}));
...
async function prepareUnoQAudio(){
  const duration=liveNoteDuration(),jobs=[];
  for(let area=0;area<activePanel.class_count;area++){
    const note=panelNote(area);if(!note)continue;const url=unoQAudioUrl(note,area,duration);if(unoQAudioBytes.has(url))continue;
    jobs.push(fetch(url,{cache:'force-cache'}).then(response=>{if(!response.ok)throw new Error(`${response.status} ${response.statusText}`);return response.arrayBuffer();}).then(bytes=>unoQAudioBytes.set(url,bytes)));
  }
  await Promise.all(jobs);
}
function scheduleUnoQAudioPrime(){if(unoQPrimeTimer)clearTimeout(unoQPrimeTimer);unoQPrimeTimer=setTimeout(()=>{const job=audioContext?primeUnoQAudio():prepareUnoQAudio();job.catch(error=>$('error').textContent=L('UNO Q音声: ','UNO Q audio: ')+error.message);},180);}
...
async function startPerformance(){
  try{await ensureAudio();let current=await api('/api/status');if(current.panel_profile_id!=='400x300x5')await api('/api/panel',{panel_profile_id:'400x300x5'});await api('/api/inference/retrigger',{milliseconds:120});if(!current.inference_active||current.device_mode!==modeForSource())await api('/api/inference/start',{mode:modeForSource()});current=await api('/api/status');lastSequence=current.latest_ai?.sequence??null;performanceEnabled=true;$('error').textContent='';performanceMessage={kind:'running'};renderPerformanceStatus();$('performanceStatus').classList.add('playing');await refreshStatus();}catch(error){$('error').textContent=error.message;}
}
...
function loadCameraSocketClient(origin){if(window.io)return Promise.resolve(window.io);if(cameraSocketClientPromise)return cameraSocketClientPromise;cameraSocketClientPromise=new Promise((resolve,reject)=>{const script=document.createElement('script');script.src=`${origin}/socket.io/socket.io.js`;script.async=true;script.onload=()=>window.io?resolve(window.io):reject(new Error(L('カメラ配信クライアントを初期化できません。','Could not initialize the camera stream client.')));script.onerror=()=>reject(new Error(L('カメラ配信クライアントを読み込めません。','Could not load the camera stream client.')));document.head.appendChild(script);}).catch(error=>{cameraSocketClientPromise=null;throw error;});return cameraSocketClientPromise;}
async function connectUnoQCamera(origin,image){const ioClient=await loadCameraSocketClient(origin);await new Promise((resolve,reject)=>{let settled=false;const socket=ioClient(origin,{transports:['websocket','polling']});cameraSocket=socket;const timer=setTimeout(()=>finish(new Error(L('UNO Qカメラ配信が応答しません。USBホスト接続を確認してください。','The UNO Q camera stream did not respond. Check the USB host connection.'))),10000);function finish(error){if(settled)return;settled=true;clearTimeout(timer);socket.off('connect_error',onError);if(error){socket.disconnect();if(cameraSocket===socket)cameraSocket=null;reject(error);}else resolve();}function onError(error){finish(new Error(error?.message||L('UNO Qカメラ配信へ接続できません。','Could not connect to the UNO Q camera stream.')));}socket.on('connect_error',onError);socket.on('connect',()=>socket.emit('hello'));socket.on('image',message=>{if(!message?.img)return;image.src=message.img;image.hidden=false;if(!settled){if(image.complete)finish();else image.onload=()=>{image.onload=null;finish();};}});});}
function releaseCamera(){if(cameraStream?.getTracks)cameraStream.getTracks().forEach(track=>track.stop());if(cameraSocket){cameraSocket.disconnect();cameraSocket=null;}cameraStream=null;const pc=$('pcCamera'),frame=$('usbCamera');pc.pause();pc.srcObject=null;pc.hidden=true;frame.onload=null;frame.onerror=null;frame.removeAttribute('src');frame.hidden=true;$('cameraPlaceholder').hidden=false;$('cameraStart').disabled=false;$('cameraStop').disabled=true;$('calibrationStart').disabled=true;$('cameraState').classList.remove('is-running');}
...
async function startCamera(){
  ...
  try{
    if(cameraSource==='pc'){
      ...
      const url=new URL($('cameraUrl').value.trim()||DEFAULT_CAMERA_STREAM);if(!['http:','https:'].includes(url.protocol))throw new Error(L('HTTPまたはHTTPSの配信URLを指定してください。','Specify an HTTP or HTTPS stream URL.'));$('cameraUrl').value=url.href;localStorage.setItem(CAMERA_KEY,url.href);const image=$('usbCamera');cameraStream=url.href;await connectUnoQCamera(url.origin,image);
    }
    cameraEnabled=true;localStorage.setItem(CAMERA_ENABLED_KEY,'true');$('cameraPlaceholder').hidden=true;$('cameraStart').disabled=true;$('cameraStop').disabled=false;$('calibrationStart').disabled=false;setCameraState('running');$('cameraState').classList.add('is-running');redrawCameraOverlay();
...
async function setupCamera(){
  $('cameraUrl').value=DEFAULT_CAMERA_STREAM;localStorage.setItem(CAMERA_KEY,DEFAULT_CAMERA_STREAM);$('cameraSource').value=cameraSource;updateCameraSourceUi();if(cameraSource==='pc')await refreshPcCameras().catch(()=>[]);setCameraState('waiting');
  $('cameraStart').onclick=startCamera;$('cameraStop').onclick=()=>{cameraEnabled=false;localStorage.setItem(CAMERA_ENABLED_KEY,'false');releaseCamera();setCameraPlaceholder('カメラはOFFです。「カメラON」で再開できます。','Camera is off. Press Camera ON to resume.');setCameraState('stopped');};
...
async function initialize(){try{const invalid=validateSongGuides();if(invalid.length)throw new Error(L('楽曲データ不整合: ','Invalid song data: ')+invalid.join(', '));await ports();let current=await api('/api/status');if(current.connected&&current.panel_profile_id!=='400x300x5')await api('/api/panel',{panel_profile_id:'400x300x5'});if(current.connected&&!current.collection?.active&&(!current.inference_active||current.device_mode!==modeForSource()))await api('/api/inference/start',{mode:modeForSource()});current=await api('/api/status');lastSequence=current.latest_ai?.sequence??null;performanceMessage={kind:'ready'};renderPerformanceStatus();$('performanceStatus').hidden=false;await refreshStatus();}catch(error){$('error').textContent=error.message;}renderPosition({x_mm:200,y_mm:150,probability_map:{support_xy_mm:[],probabilities:[]},class_probabilities:Array(12).fill(1/12)},false);setupCamera().then(()=>{if(cameraEnabled)return startCamera();setCameraPlaceholder('カメラはOFFです。「カメラON」で再開できます。','Camera is off. Press Camera ON to resume.');setCameraState('stopped');}).catch(error=>setCameraPlaceholder(cameraError(error),cameraError(error)));prepareUnoQAudio().catch(error=>$('error').textContent=L('UNO Q音声準備: ','UNO Q audio preparation: ')+error.message);inferenceLoop();}
