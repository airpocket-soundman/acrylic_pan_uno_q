...
const CAMERA_KEY = 'acrylicPanPcCameraStreamUrl';
const DEFAULT_CAMERA_STREAM = 'http://192.168.50.177:8878/stream.mjpg';
...
let cameraPlaceholderText = ['PCカメラ配信へ接続してください','Connect to the PC camera stream.'];
...
function applyLanguage(){
  ...
  [['panelProfileLabel','板仕様','Panel'],['portLabel','COMポート','COM port'],['sourceLabel','確率推論元','Probability source'],['displayLabel','表示','Display'],['instrumentLabel','音色','Instrument'],['masterVolumeLabel','音量','Volume'],['cameraLabel','PC配信URL','PC stream URL'],['songLabel','演奏補助','Performance guide'],['noteLengthLabel','音の長さ','Note length']].forEach(args=>setLabelText(args[0],L(args[1],args[2])));
  [['refresh','更新','Refresh'],['connect','接続','Connect'],['disconnect','切断','Disconnect'],['performanceStart','演奏開始','Start performance'],['performanceStop','演奏停止','Stop performance'],['probabilityDemo','表示デモ','Display demo'],['cameraStart','配信接続','Connect stream'],['cameraStop','停止','Stop'],['calibrationStart','4点位置合わせ','4-point alignment'],['calibrationClear','位置合わせ消去','Clear alignment'],['songReset','最初から','Restart']].forEach(([id,ja,en])=>$(id).textContent=L(ja,en));
...
function loadCalibration(){try{const points=JSON.parse(localStorage.getItem(CAMERA_CALIBRATION_KEY)||'[]');return Array.isArray(points)&&points.length===4&&points.every(point=>Array.isArray(point)&&point.length===2&&point.every(Number.isFinite))?points:[[0,0],[1,0],[1,1],[0,1]];}catch(_){return [[0,0],[1,0],[1,1],[0,1]];}}
...
function releaseCamera(){cameraStream=null;$('usbCamera').removeAttribute('src');$('cameraPlaceholder').hidden=false;$('cameraStart').disabled=false;$('cameraStop').disabled=true;$('calibrationStart').disabled=true;$('cameraState').classList.remove('is-running');}
async function startCamera(){
  releaseCamera();setCameraState('connecting');
  try{
    const url=new URL($('cameraDevice').value.trim()||DEFAULT_CAMERA_STREAM);
    if(!['http:','https:'].includes(url.protocol))throw new Error(L('HTTPまたはHTTPSの配信URLを指定してください。','Specify an HTTP or HTTPS stream URL.'));
    $('cameraDevice').value=url.href;localStorage.setItem(CAMERA_KEY,url.href);
    const image=$('usbCamera');cameraStream=url.href;
    await new Promise((resolve,reject)=>{const timer=setTimeout(()=>reject(new Error(L('PCカメラ配信が応答しません。','PC camera stream did not respond.'))),6000);image.onload=()=>{clearTimeout(timer);resolve();};image.onerror=()=>{clearTimeout(timer);reject(new Error(L('PCカメラ配信を読み込めません。','Could not load the PC camera stream.')));};image.src=url.href;});
  ...
  }catch(error){releaseCamera();setCameraState('failed');setCameraPlaceholder(error.message,error.message);}
}
async function setupCamera(){
  $('cameraDevice').value=localStorage.getItem(CAMERA_KEY)||DEFAULT_CAMERA_STREAM;setCameraState('waiting');
  $('cameraStart').onclick=startCamera;$('cameraStop').onclick=()=>{releaseCamera();setCameraState('stopped');};
  $('cameraDevice').onchange=()=>localStorage.setItem(CAMERA_KEY,$('cameraDevice').value.trim());
