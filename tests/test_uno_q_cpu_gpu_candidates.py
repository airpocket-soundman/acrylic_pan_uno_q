...
class UnoQCpuGpuCandidateTests(unittest.TestCase):
    def test_selected_models_match_metadata_and_accuracy_contract(self):
        ...
        self.assertEqual(report["split"]["counts"], {"train": 4745, "validation": 480, "test": 1907})
        ...
        self.assertGreater(report["tflite_test"]["cpu_dynamic"]["position_top1_accuracy"], .96)
        self.assertGreater(report["tflite_test"]["gpu_fp16"]["position_top1_accuracy"], .97)
        self.assertGreater(report["tflite_test"]["cpu_dynamic"]["area_accuracy_12class"], .98)
        self.assertGreater(report["tflite_test"]["gpu_fp16"]["area_accuracy_12class"], .98)
