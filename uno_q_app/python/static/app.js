...
function drawCollection(collection) {
  ...
  if (collection.active) {
    ...
    summary.textContent = collection.completed_samples > 0 ?
  ...
  $('collectionStart').textContent = collection.active ? '採取中' : '採取開始';
...
if ($('libraryList')) {
  ...
  $('libraryDeleteSession').onclick = async () => {
    if (!library.sessionId) return;
    if (!confirm(`採取データ ${library.sessionId} をすべて削除します。\n` +
...
if ($('collectionPattern')) $('collectionPattern').onchange = async () => {
  try {
    $('collectionRepetitions').value = $('collectionPattern').value === 'corners' ? 10 : 50;
