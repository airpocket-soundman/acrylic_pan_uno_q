...
async function startCamera() {
  ...
  try {
    ...
    $('cameraDevice').value = url.href;
    localStorage.setItem(CAMERA_STORAGE_KEY, url.href);
    const frame = $('usbCamera');
    cameraStream = url.href;
    await new Promise((resolve, reject) => {
      const timer = setTimeout(() => reject(new Error('UNO Qカメラ配信が応答しません。USBホスト接続を確認してください。')), 10000);
      frame.onload = () => { clearTimeout(timer); resolve(); };
      frame.onerror = () => { clearTimeout(timer); reject(new Error('UNO Qカメラ配信を読み込めません。')); };
      frame.src = url.href;
    });
    ...
    $('cameraPlaceholder').textContent = `カメラを開始できません: ${error?.message || String(error)}`;
    throw error;
  }
}

async function setupCamera() {
  $('cameraDevice').value = localStorage.getItem(CAMERA_STORAGE_KEY) || DEFAULT_CAMERA_STREAM;
  $('cameraState').textContent = '開始待ち';
